...
class MatchOneLayer(nn.Module):
    def __init__(self, input_size, output_size, num_models):
        ...
        self.global_model = nn.Parameter(torch.rand(self.input_size, self.output_size))
        self.log_softmax = nn.LogSoftmax(dim=1)
        self.pdist = nn.PairwiseDistance(p=2)
        self.softmax = nn.Softmax(dim=2)
        self.loss = SamplesLoss("sinkhorn", p=2, blur=.1)
        self.sinkhorn = SinkhornDistance(eps=1, max_iter=100, reduction=None)
    ...
    def forward(self, layer_weights):
        ...
        loss, P, C = self.sinkhorn(layer_weights.permute(0,2,1), self.global_model.T.repeat(self.num_models, 1, 1))
        # p0 = torch.zeros_like(P)

        # transported_layer_weights = torch.matmul(layer_weights, P*self.output_size)
        # global_layer_weights = (torch.sum(transported_layer_weights,axis=0) / self.num_models)
        # loss = self.pdist(transported_layer_weights, global_layer_weights.repeat(self.num_models, 1, 1)).sum()
        # for sub_matrix in transported_layer_weights:
        #     loss += self.loss(global_layer_weights, sub_matrix)
        ...
        post_loss = loss.sum() #+ squared_col_diff #+ squared_row_diff + squared_col_diff

        return post_loss, [loss, 0, 0], self.global_model.detach(), P
...
def get_matched_weights(cur_weights, layer_idx, args):
    ...
    for epoch in range(args.diff_match_epochs):
        ...
        transported_layer_weights = torch.matmul(layer_weights, pi_li*match.output_size)
        global_layer_weights = (torch.sum(transported_layer_weights,axis=0) / 2)

        if (epoch+1) % 10 == 0 or epoch == 0:
            logging.debug(f'Epoch {epoch+1:5}')
            logging.debug(f'Loss: {loss}')
            logging.debug(f'pdist: {[torch.nn.PairwiseDistance(p=2)(global_layer_weights.T, x.T).sum() for x in transported_layer_weights]}')
    ...
    pi_li_ones = torch.zeros(2,100,100)
    pi_li_ones[[0,1], torch.stack([torch.arange(0,100), torch.arange(0,100)]).T, pi_li.argmax(0).T] = 1
